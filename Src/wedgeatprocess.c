...
uint8_t Setpara(uint8_t * dataBuf1,char buffer[255][255],int length);
...
{
    AppAtMainProcess(SmsData,DataLen,ToSMS);
}
...
int smssend(u8 PYLDType, char *fmt, ...)
{
	int cnt;
        ...
        if (cnt > 0)
        {
            ...
            {
                WedgeResponseSms(PYLDType,string,cnt);
            }
            else
            {
                WedgeResponseSms(PYLDType,string,MAX_PRINTF_STR_SIZE);
            }
        }
        va_end(ap);
				memset(imei,0,15);
...
void RemoveAT(uint8_t * dataBuf,uint8_t  dataLen,uint8_t * dataBuf1,uint8_t dataLen1)
{
    ...
    {
        memset( (char *)dataBuf1, 0, sizeof(dataBuf1));
        dataLen1=0;
        if ((dataBuf[0]=='a'||dataBuf[0]=='A')&&(dataBuf[1]=='t'||dataBuf[1]=='T'))
        {
            for ( idx = 0; idx < dataLen-2 ; idx++ )
            {
                ...
                if ( 1 )
                {
                    dataBuf1[dataLen1++] = dataBuf[idx+2];
                }
                else
                {
                    break;
...
void AdmSMSSet(WEDGEPYLDTypeDef PYid,ATCmdSource_t type,u8 *buffer)
{
    ...
    {
        strncpy(SMS->sms_1, str[0], strlen(str[0]));

        strncpy(SMS->sms_2, str[1], strlen(str[1]));

        strncpy(SMS->sms_3, str[2], strlen(str[2]));
...
void AdmSVRCFGSet(ATCmdSource_t type,WEDGEPYLDTypeDef PYid,u8 *buffer)
{
    ...
    if (cnt>0)
    {
        ...
        if ((cnt-3)>0)
        {
            strncpy(SVRCFG->srvr1, str[3], strlen(str[3]));
            WedgeCfgSet(WEDGE_CFG_SVRCFG,SVRCFG);
        ...
        if ((cnt-3)>1)
        {
            strncpy(SVRCFG->srvr2, str[4], strlen(str[4]));
            WedgeCfgSet(WEDGE_CFG_SVRCFG,SVRCFG);
        ...
        if ((cnt-3)>2)
        {
            strncpy(SVRCFG->srvr3, str[5], strlen(str[5]));
            WedgeCfgSet(WEDGE_CFG_SVRCFG,SVRCFG);
            WedgeCfgChgStateSet(SVRCFG_CFG_CHG_3,TRUE);
        }
        if ((cnt-3)>3)
        {
            strncpy(SVRCFG->srvr4, str[6], strlen(str[6]));
            WedgeCfgSet(WEDGE_CFG_SVRCFG,SVRCFG);
            WedgeCfgChgStateSet(SVRCFG_CFG_CHG_4,TRUE);
        }
        if ((cnt-3)>4)
        {
            strncpy(SVRCFG->srvr5, str[7], strlen(str[7]));
            WedgeCfgSet(WEDGE_CFG_SVRCFG,SVRCFG);
...
void AdmAPNCFGSet(ATCmdSource_t type,WEDGEPYLDTypeDef PYid,u8 *buffer)
{
    ...
    if (Setpara(buffer,str,3)>0)
    {
        if (1)
        {
            strncpy(APNCFG->apn, str[0], strlen(str[0]));
        }
        if (1)
        {
            strncpy(APNCFG->usr, str[1], strlen(str[1]));
        }
        if (1)
        {
            strncpy(APNCFG->pwd, str[2], strlen(str[2]));
        }
...
void AdmUSRDATSet(ATCmdSource_t type,WEDGEPYLDTypeDef PYid,u8 *buffer)
{
    ...
    if (Setpara(buffer,str,1)>0)
    {
        strncpy(USRDAT->data, str[0], strlen(str[0]));
...
void ATGetProcess(ATCmdType_t type,ATCmdSource_t DataSource,WEDGEPYLDTypeDef PYid,char IMEI[15])
{
    switch (type)
    {
            ...
            while ( result != NULL )
            {
                ...
                if (strchr(result, '='))
                {
									 if (1)
                    {
                        ...
                        {
                            matchCmdType=ATCmdGetType1((ATCompareTable1_t *)ATCmdTypeTable, (sizeof(ATCmdTypeTable)/sizeof(ATCmdTypeTable[0])), CMD, strlen(CMD), &idxTemp);
                            ...
                            ATSetProcess(matchCmdType, Fromtype, PYID, p,IMEI);
                        }
                    }
                }
                else
                {
                    ...
                    if (p && 1==sscanf(p,"*%255[^?]",CMD))
                    {
                        matchCmdType=ATCmdGetType1((ATCompareTable1_t *)ATCmdTypeTable, (sizeof(ATCmdTypeTable)/sizeof(ATCmdTypeTable[0])), CMD, sizeof(CMD), &idxTemp);
                        memset( (char *)CMD, 0, sizeof(CMD));
                        if (matchCmdType==0xff)
                        {
                            SendRpt(PYID,Fromtype,"ERROR1");
                        ...
                        ATGetProcess(matchCmdType,Fromtype, PYID,IMEI);
...
char * mystrtok(char * s, const char * ct)
{
    ...
    sbegin += strspn(sbegin, ct);//����??��?��?��??D�̨���???2??��???����?��?��??D3???��?��?��???����
    if (*sbegin == '\0') {
        myStrtok = NULL;
        return (NULL);
    }
